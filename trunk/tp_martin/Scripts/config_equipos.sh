...
# Definicion de todas las constantes para configurar subredes, hosts, servers, dns, etc 
RED_A=10.94.5.128 
...
CONDOR1_RED_E=10.94.5.195 
HALCON1_RED_I=10.94.6.148 
PATO1_RED_P=10.43.10.4 
...
# Se define asi para que sea global y luego se cambia su valor desde una funcion 
INTERFAZ=""  
...
function Log(){ 
	# nos sirve para casos donde queremos mas info de lo que  
	# esta pasando. Para que se vea info la variable 
	# DEBUG se debe setear en 1 
	if [ $DEBUG = 1 ]; then 
		echo "[DEBUG] - $1" 
	fi 
} 
...
function DeshabilidarInterfaces(){ 
	Log "Deshabilitando interfaces en el sistema..." 
...
function GetInterfaz() { 
	...
	# para trabajar utilizamos solo la primer interfaz ethernet del equipo 
	INTERFAZ=`ifconfig -a | grep -m1 "^eth.*" | awk {'print $1'}` 
	Log "Se inicia unicamente la interfaz con la que trabajaremos ($INTERFAZ)"  
...
function ConfigurarDNS() { 
	...
	case "$1" in	     
			SANRAFAEL)  
			Log "Ejecutando: echo search mendoza.dc.fi.uba.ar sanrafael.mendoza.dc.fi.uba.ar tupungato.mendoza.dc.fi.uba.ar laslenas.mendoza.dc.fi.uba.ar > /etc/resolv.conf" 
			echo "search mendoza.dc.fi.uba.ar sanrafael.mendoza.dc.fi.uba.ar tupungato.mendoza.dc.fi.uba.ar laslenas.mendoza.dc.fi.uba.ar" > /etc/resolv.conf		 
...
function InicializarConfiguracion() { 
	...
	echo "Iniciando configuracion para dispositivo $1..." 
 
	Log "Ejecutando InicializarConfiguracion" 
 
	DeshabilidarInterfaces 
...
function main() { 
#TODO: buscar los ciclos para las rutas de contingencia 
	case "$1" in	     
			H3) 
			...
			CONDOR1) 
				...
				ConfigurarInterfaz $INTERFAZ $CONDOR1_RED_E $MASCARA_E 
			...
			HALCON1) 
				...
				ConfigurarInterfaz $INTERFAZ $HALCON1_RED_I $MASCARA_I 
			...
			PATO1) 
				...
				ConfigurarInterfaz $INTERFAZ $PATO1_RED_P $MASCARA_P 
...
if [ $# = 0 ]; then 
	...
	echo "HOSTS: CONDOR1, HALCON1, PATO1" 
	exit -1 
elif [ $# = 2 ]; then	 
	DEBUG=$2 
fi 
